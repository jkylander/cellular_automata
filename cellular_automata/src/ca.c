...
void update_cells(void) {
  ...
  nextCells[TOTAL_CELLS - 1] =
      calculate_state(cells[TOTAL_CELLS - 2], cells[TOTAL_CELLS - 1], cells[0]);
...
int main(void) {
  ...
  while (!WindowShouldClose()) {
    ...
    case TITLE: {
      ...
      /* RLAPI Vector2 MeasureTextEx(Font font, const char *text, float
       * fontSize, float spacing);    // Measure string size for Font */
      DrawText(title, MIDDLE(SCREEN_WIDTH, MeasureText(title, 40)),
               SCREEN_HEIGHT / 4.0f - 100, 40, DARKGREEN);
      DrawText(description, MIDDLE(SCREEN_WIDTH, MeasureText(description, 30)),
